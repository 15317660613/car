                ...
                <div class="newsparent" style="">
                    <div style="display: flex;flex-flow: column;" class="chartParent">
                        ...
                        <div style="border-radius: 4px;border: 1px solid #DCDFE6;margin-top: 20px">
                            <div class="chartTitle" style="display: flex;justify-content: space-between;">
                                <span>{{provinceYear}}二手乘用车交易量省份表现</span>
                                <span style="margin-right: 20px;">单位：辆</span>
                            </div>
                            <v-chart :options="polarProvince" class="chartHeight"/>
                        </div>
                    </div>
...
<script>
    import {
        getInformation,
        getStatUsedCarSaleList,
        getStatUsedCarProvinceList,
    } from '@/api/stat'
    ...
    export default {
        ...
        created() {
            this.getInformation();
            this.getStatUsedCarSaleList()
            this.getStatUsedCarProvinceList()
        },
        mounted() {
        ...
        methods: {
            ...
            //二手乘用车销量走势
            getStatUsedCarSaleList() {
                getStatUsedCarSaleList({}).then(response => {
                    let _this = this
                    if (response.data.now && response.data.now.length != 0) {
                        _this.ageYear = response.data.year + '年'
                        response.data.now.forEach(function (item) {
                            _this.polarAge.xAxis.data.push(item.MONTH)
                            _this.polarAge.series[0].data.push(item.TRADING_VOLUME)
                            _this.polarAge.series[1].data.push(item.volume.replace('%', ''))
                            _this.polarAge.series[0].name = '销量';
                            _this.polarAge.series[1].name = '同比';
                        });
            ...
            //二手车省份
            getStatUsedCarProvinceList() {
                getStatUsedCarProvinceList({}).then(response => {
                    this.provinceYear = response.data.year + '年'
                    let ageMaps = []
                    if (response.data.map && response.data.map.length != 0) {
                        response.data.map.forEach(function (item) {
                            var index = []
                            index.push(item.PROVINCE)
                            index.push(item.SUM)
                            ageMaps.push(index)
                        });
                        this.polarProvince.series[0].data = ageMaps
