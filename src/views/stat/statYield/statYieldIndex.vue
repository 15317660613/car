                ...
                <div class="newsparent" style="">
                    <div style="display: flex;flex-flow: column;" class="chartParent">
                        ...
                        <div style="border-radius: 4px;border: 1px solid #DCDFE6;margin-top: 20px">
                            <div class="chartTitle" style="display: flex;justify-content: space-between;">
                                <span>{{companyYear}}汽车市场TOP10企业产量表现</span>
                                <span style="margin-right: 20px;">单位：辆</span>
                            </div>
...
<script>
    import {
        getInformation,
        getStatMonthYieldList,
        getStatEnterpriseYieldList,
    } from '@/api/stat'
    ...
    export default {
                    ...
                    tooltip: {
                        ...
                        formatter: function (params) {
                            ...
                            params.forEach(item=>{
                                ...
                                if(item.seriesName.indexOf('增速')!=-1){
                                    item.data += '%';
                                }
        ...
        created() {
            this.getInformation();
            this.getStatMonthYieldList()
            this.getStatEnterpriseYieldList()
        },
        mounted() {
        ...
        methods: {
            ...
            //保有量车龄分布
            getStatMonthYieldList() {
                getStatMonthYieldList({}).then(response => {
                    this.ageYear = response.data.year
                    let _this = this
                    if (response.data.maps && response.data.maps.length != 0) {
                        this.polarAge.xAxis.data = response.data.maps[0].monthData
                        this.polarAge.series[0].data = response.data.maps[0].last
                        this.polarAge.series[0].name = (parseInt(_this.ageYear) - 1) + '年'

                        this.polarAge.series[1].data = response.data.maps[0].curr
                        this.polarAge.series[1].name = _this.ageYear + '年'

                        this.polarAge.series[2].name = _this.ageYear + '年增速'
                        response.data.maps[0].compara.forEach(function (item) {
                            _this.polarAge.series[2].data.push(item.replace('%', ''))
                        });
                    }
                })
            },
            //产量企业
            getStatEnterpriseYieldList() {
                getStatEnterpriseYieldList({}).then(response => {
                    this.companyYear = response.data.month + '月'
                    let ageMaps = []
                    if (response.data.maps && response.data.maps.length != 0) {
                        response.data.maps.forEach(function (item) {
                            var index = []
                            index.push(item.COMPANYNAME)
                            index.push(item.SUM)
                            ageMaps.push(index)
                        });
