                ...
                <div class="newsparent" style="">
                    <div style="display: flex;flex-flow: column;" class="chartParent">
                        ...
                        <div style="border-radius: 4px;border: 1px solid #DCDFE6;margin-top: 20px">
                            <div class="chartTitle" style="display: flex;justify-content: space-between;">
                                <span>{{companyYear}}主流企业乘用车保有量</span>
                                <span style="margin-right: 20px;">单位：辆</span>
                            </div>
...
<script>
    ...
    export default {
        ...
        created() {
            this.getInformation();
            this.getStatStockAgeList()
            this.getStatStockCompanyList()
        },
        mounted() {
        ...
        methods: {
            ...
            //保有量车龄分布
            getStatStockAgeList() {
                getStatStockAgeList({}).then(response => {
                    this.ageYear = response.data.year + '年'
                    let ageMaps = []
                    if (response.data.maps && response.data.maps.length != 0) {
                        response.data.maps.forEach(function (item) {
                            var index = []
                            index.push(item.VEHICLEAGE)
                            index.push(item.SUM)
                            ageMaps.push(index)
                        });
                        this.polarAge.series[0].data = ageMaps
            ...
            //保有量企业
            getStatStockCompanyList() {
                getStatStockCompanyList({}).then(response => {
                    this.companyYear = response.data.year + '年'
                    let ageMaps = []
                    if (response.data.maps && response.data.maps.length != 0) {
                        response.data.maps.forEach(function (item) {
                            var index = []
