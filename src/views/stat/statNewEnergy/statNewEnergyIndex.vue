                    ...
                    <div style="display: flex;flex-flow: column;" class="chartParent">
                        <div style="border-radius: 4px;border: 1px solid #DCDFE6;">
                            <div class="chartTitle" style="display: flex;justify-content: space-between;">
                                <span>{{companyYear}}新能源市场月度销量</span>
                        ...
                        <div style="border-radius: 4px;border: 1px solid #DCDFE6;margin-top: 20px">
                            <div class="chartTitle" style="display: flex;justify-content: space-between;">
                                <span>{{companyYear}}新能源TOP10企业产量表现</span>
                                <span style="margin-right: 20px;">单位：辆</span>
                            </div>
                            <v-chart :options="polarCompany" class="chartHeight"/>
                        </div>
                    </div>
...
<script>
    import {
        getInformation,
        getStatNewEnergyMonthList,
        getStatNewEnergyCompanyList,
    } from '@/api/stat'
    ...
    export default {
                ...
                polarCompany: {
                    ...
                    tooltip: {
                        ...
                        formatter: function (params) {
                            ...
                            return firstParams.data[0] + '：' + firstParams.data[1];
                        }
                    },
                    grid: {
                        left: '3%',
        ...
        created() {
            this.getInformation();
            this.getStatNewEnergyMonthList()
            this.getStatNewEnergyCompanyList()
        },
        mounted() {
        ...
        methods: {
            ...
            //保有量车龄分布
            getStatNewEnergyMonthList() {
                getStatNewEnergyMonthList({}).then(response => {
                    this.ageYear = response.data.year
                    let _this = this
                    if (response.data) {
                        console.log(response.data);
                        if (response.data.now && response.data.now.length != 0) {
                            this.companyYear = response.data.now[0].year + '年'
                            this.polarAge.series[0].name = (parseInt(response.data.now[0].year) - 1) + '年' + response.data.now[0].month + '月'
                            this.polarAge.series[1].name = response.data.now[0].year + '年' + response.data.now[0].month + '月'

                            response.data.now.forEach(function (item) {
                                //x轴数据赋值
                                _this.polarAge.xAxis.data.push(item.TYPE)
                                _this.polarAge.series[1].data.push(item.SUM ? item.SUM : 0)
                            });

                            response.data.last.forEach(function (item) {
                                _this.polarAge.series[0].data.push(item.SUM ? item.SUM : 0)
                            });
                        }

                        this.polarAge.series[2].name = '同比'
                        response.data.yearOnYear.forEach(function (item) {
                            _this.polarAge.series[2].data.push(item)
                        });
                    }
                })
            },
            //产量企业
            getStatNewEnergyCompanyList() {
                getStatNewEnergyCompanyList({}).then(response => {
                    let ageMaps = []
                    if (response.data && response.data.length != 0) {
                        this.companyYear = response.data[0].YEAR + '年'
                        response.data.forEach(function (item) {
                            var index = []
                            index.push(item.COMPANY_NAME)
                            index.push(item.SUM)
                            ageMaps.push(index)
                        });
                        this.polarCompany.series[0].data = ageMaps
